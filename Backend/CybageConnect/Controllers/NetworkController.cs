...
namespace CybageConnect.Controllers
{
    ...
    public class NetworkController : ControllerBase
    {
        ...
        [ProducesResponseType(StatusCodes.Status404NotFound)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        public async Task<IActionResult> DeleteConnection(ConnectionDTO connection)
        {
            try
            {
                var result = await _networkService.DeleteConnection(connection.UserId, connection.FriendId);
                return Ok(result);
            }
            catch (Exception ex)
            {
